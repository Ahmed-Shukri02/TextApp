...
export default function ContentPost({
  ...
  */
  ...
  useEffect(() => {
    ...
    return () => {
      if (thisPost.current) {
        thisPost.current.removeEventListener("mouseenter", () =>
          setIsHovering(true)
        );
        thisPost.current.removeEventListener("mouseleave", () =>
          setIsHovering(false)
        );
      }
    };
  ...
  */
  ...
  const repliesJSX =
    ...
    replies.map((elem, index) => (
      <Reply
        index={index}
        removeReply={popRepliesAtIndex}
        info={elem}
        userInfo={userInfo}
        key={elem.reply_id}
        loadedImages={loadedImages}
        commentBoxReference={commentBoxReference}
        toggleCommentBox={handleCommentingToReply}
        postInfo={postInfo}
      />
    ));
  ...
  function personDetails() {
    return (
      <div
        data-testid="person-detail-flex"
        className="person-detail-flex"
        ref={thisPost}
      >
        <div style={{ display: "flex", gap: "1em", alignItems: "center" }}>
          <div className="person-detail-image">
            {!userInfo.user_pfp ? (
              loadedImages(userInfo.stock_pfp)
            ) : (
              <img
                className="media"
                src={
          ...
          <div className="person-detail-info">
            <Buttons.UnderlineButton
              handleClick={navigateToUser}
              addStyle="no-padding"
            >
              <div className="post-author">
                {userInfo.username}{" "}
                {userInfo.is_verified && <IconComponents.Checkmark />}{" "}
              </div>
            </Buttons.UnderlineButton>
        ...
        <div data-testid="delete-options" className="deleteOptions">
          {clientOwns && isHovering && (
            <Buttons.DefaultButton
              theme="white"
              handleClick={() => deletePost()}
            >
              <IconComponents.TrashIcon iconClass="delete-icon" />
            </Buttons.DefaultButton>
          )}
        </div>
      </div>
  ...
  function post() {
    return (
      <div className="post">
        {postInfo.post_text && (
          <div className="post-content">{postInfo.post_text}</div>
        )}
        {mediaJSX && mediaJSX[0]}
      </div>
    );
  ...
  function Interaction() {
    ...
    function seeMore(num) {
      ...
      let newVal =
        commentsLength + num > replies.length
          ? replies.length
          : commentsLength + num;
          ...
          <div className="interaction-prompt">
            <div className="interaction-prompt-left">
              {isLiked ? (
                ...
                >
                  <IconComponents.ThumbUpIcon fill="#1B74E4" stroke="black" />{" "}
                  {likes}{" "}
                </Buttons.DefaultButton>
              ) : (
                <Buttons.DefaultButton
                  ...
                  isLoading={likeLoading}
                >
                  <IconComponents.ThumbUpIcon /> {likes}{" "}
                </Buttons.DefaultButton>
              )}
              <Buttons.DefaultButton
                testid="chat-button"
              ...
              >
                <IconComponents.ChatBubbleIcon /> {replies.length}{" "}
              </Buttons.DefaultButton>

              <Buttons.DefaultButton {...buttonStyle}>
                <IconComponents.ArrowIcon /> 0{" "}
              </Buttons.DefaultButton>
            </div>
            <div className="interaction-prompt-right">
              {replies.length > 0 && (
                <Buttons.UnderlineButton
                  handleClick={() => setShowReply((oldVal) => !oldVal)}
                  fontSize="inherit"
                  contentColor="#1B74E4"
                  theme="blue"
                >
            ...
            <div className="replies">
              <div className="most-relevant">
                ...
                <div style={{ display: "flex", alignItems: "center" }}>
                  Most recent <IconComponents.ExpandDownIcon />
                </div>
              </div>
              ...
              <div className="comments-displaying">
                Displaying{" "}
                <span>
                  {commentsLength} out of {replies.length}
                </span>{" "}
                comments
              </div>
              ...
              <div className="see-more-container">
                <div className="see-more-less">
                  ...
                  {commentsLength > 1 && (
                    <Buttons.DefaultButton
                      width="7em"
                      handleClick={() => setCommentsLength(1)}
                      fontSize="0.8rem"
                      contentColor="white"
                    >
                      {" "}
  ...
  return (
    <div data-testid="post" className="content-post">
      {personDetails()}
      {post()}
      {Interaction()}
    </div>
